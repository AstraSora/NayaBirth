      ...
      <div className="min-h-screen bg-gradient-warm flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <h1 className="text-xl font-bold text-foreground mb-2">Category not found</h1>
            <Link
              to="/resources"
      ...
      <main className="flex-1 max-w-lg mx-auto px-4 py-6 w-full">
        ...
        {!category.heroImage && (
          <div className="text-center mb-6">
            ...
            <p className="text-foreground-secondary text-sm">{category.description}</p>
          </div>
        )}
          ...
          {category.resources.map((resource) => (
            <article key={resource.id} className="bg-surface rounded-2xl shadow-card overflow-hidden">
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-foreground-secondary">{resource.summary}</p>
              ...
              {expandedArticle === resource.id && (
                <div id={`article-${resource.id}`} className="border-t border-subtle animate-fade-in">
                  <div className="p-5 pt-4">
                    ...
                    <div className="prose prose-sm max-w-none">
                      {resource.content.split('\n\n').map((paragraph, index) => {
                        ...
                        if (hasListItems) {
                          ...
                          lines.forEach((line, lineIndex) => {
                            ...
                            if (trimmedLine.startsWith('**') && trimmedLine.endsWith('**')) {
                              ...
                              if (currentList.length > 0) {
                                elements.push(
                                  <ul key={`list-${lineIndex}`} className="list-disc list-inside text-foreground-secondary space-y-1 my-2">
                            ...
                            // Regular text line
                              ...
                              elements.push(
                                <p key={`p-${lineIndex}`} className="text-foreground-secondary my-1" dangerouslySetInnerHTML={{
                                  __html: parseMarkdown(trimmedLine)
                                }} />
                              )
                            }
                          ...
                          if (currentList.length > 0) {
                            elements.push(
                              <ul key={`list-final`} className="list-disc list-inside text-foreground-secondary space-y-1 my-2">
                                {currentList.map((item, i) => (
                                  <li key={i} dangerouslySetInnerHTML={{
                                    __html: parseMarkdown(item)
                                  }} />
                        ...
                        return (
                          <p
                            key={index}
                            className="text-foreground-secondary my-2"
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-subtle">
                      {resource.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-xs px-2 py-1 bg-muted text-foreground-muted rounded-full"
                        >
                          {tag}
                        </span>
                      ))}
